...
pub fn main() {
  ...
  while counter < max {
    ...
    } else if current_value == 3 { // flagged
      // reverse
      dx = -dx;
      dy = -dy;
    }

    if current_value == 0 { // clean
      map[current_y][current_x] = 1;
    } else if current_value == 1 { // weakened
      map[current_y][current_x] = 2;
      answer += 1;
    } else if current_value == 2 { // infected
